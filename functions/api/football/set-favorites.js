...
export async function onRequestPost(context) {
    ...
    if (!DB || !JWT_SECRET) return new Response(JSON.stringify({ error: "Sunucu yapılandırması eksik." }), { status: 500 });
     ...
     try {
        ...
        if (!authHeader || !authHeader.startsWith('Bearer ')) return new Response(JSON.stringify({ error: 'Giriş yapmanız gerekiyor.' }), { status: 401 });
        ...
        if (!Array.isArray(favoriteIds)) return new Response(JSON.stringify({ error: 'Geçersiz veri formatı.' }), { status: 400 });
